...
export function LabelFilter() {
  ...
  return (
    ...
    >
      ...
      <Select.Portal>
        <Select.Positioner side="bottom" align="end" sideOffset={8} alignItemWithTrigger={false}>
...
function LabelItem({ value, name }: { value: string | null; name: string }) {
  return (
    <Select.Item
      value={value}
      className="transition-all flex items-center justify-between gap-2 rounded-lg px-2 py-1.5 text-sm cursor-default data-highlighted:bg-accent-hover/60"
    >
