...
A local-first journaling application with client-side SQLite storage. Data stays on the device with manual export/import for backups.

This repository contains only the client-side SPA.
...
- `src/db/merger.ts` - Merge logic (dump/merge functions for import)
